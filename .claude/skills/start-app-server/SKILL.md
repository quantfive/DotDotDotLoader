...
Fast-path skill for starting DotDotDotLoader's app server. All discovery was done during bootstrap on 2026-05-07T00:45:00Z — just execute.
...
- **Stack**: Node.js Express serving a pre-built React app on port 3000
...
- Existing Dockerfile copies a pre-built `/build` folder that is gitignored; `Dockerfile.codepress` runs `npm run build` inside the image to generate it before serving.
